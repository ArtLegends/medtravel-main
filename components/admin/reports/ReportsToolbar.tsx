...
import { deleteAllReportsAction } from '@/app/(admin)/admin/reports/actions'
import Link from 'next/link'

export default function ReportsToolbar({ start, end }: { start?: string; end?: string }) {
  const [isPending, startTransition] = useTransition()
  const router = useRouter()

  function onDeleteAll() {
    const c1 = confirm('Delete ALL reports? This cannot be undone.')
    if (!c1) return
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
      <form method="GET" className="flex flex-wrap items-end gap-3">
        ...
        <Link href="/admin/reports" className="rounded-md border px-4 py-2 hover:bg-gray-50">
          Clear Filters
        </Link>
      </form>
