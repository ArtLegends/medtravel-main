...
### Пуш обновлений в production

- `git add .`
- `git commit -m "название коммита"`
- `git push`

Последовательное выполнение команд в терминале.

### Таблицы

- `profiles` и `user_roles` - таблицы пользователей в схеме public, в profiles отображается последняя используемая пользователем роль, в user_roles задаются любые роли к uuid пользователя, также и роль админа, чтобы дать пользователю роль админ нужно в таблице user_roles добавить запись указав uuid пользователя и роль admin.

### Метатеги

Находятся в `app/layout.tsx` и `lib\seo\meta.ts`(в мета файле используются метатеги для клиник, категорий и фильтров), а также остальные файлы page.tsx используют экспорт `metadata` или `generateMetadata()`.